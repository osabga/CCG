...
function getMonth(date: string): { year: number, month: number } {
...
export async function getData(req: Request,res:Response){
    ...
    if (!tipo || (tipo !== "usuarios" && tipo !== "preguntas")) {
        return res.status(403).json({ message: "Tipo de datos faltantes o inválido" });
    }

    let query: any = {};

    if (body.date_begin && body.date_end) {
        query.date = { $gte: new Date(body.date_begin), $lte: new Date(body.date_end) };
    }
...
export async function getMonthly(req:Request,res:Response){
    ...
    if (!tipo || (tipo !== "usuarios" && tipo !== "preguntas")) {
        return res.status(403).json({ message: "Tipo  faltantes o inválido" });
    }
    ...
    const month_begin = getMonth(body.date_begin);
    const month_end = getMonth(body.date_end);
...
export async function getMonthly(req: Request, res: Response) {
    ...
    while (currentYear < endDate.getFullYear() || (currentYear === endDate.getFullYear() && currentMonth <= (endDate.getMonth() + 1))) {
        ...
        const request = {
            params: { tipo },
            body: { date_begin: monthStart, date_end: monthEndStr }
        };

        try {
            const json_data = await getData(request as unknown as Request, res);
...
// New function to get the data count
